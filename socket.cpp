...
std::string get_endpoint_address_(const Ed& ed)
{
    ...
    if (ec)
    {
        LOG_ERROR("get endpoint address failed {}", ec.message());
        return "";
    }
...
static std::string get_socket_local_ip_(Socket& socket)
{
    ...
    if (ec)
    {
        LOG_ERROR("get socket local address failed {}", ec.message());
        return "";
    }
...
static uint16_t get_socket_local_port_(Socket& socket)
{
    ...
    if (ec)
    {
        LOG_ERROR("get socket local port failed {}", ec.message());
        return 0;
    }
    return ed.port();
}
...
static std::string get_socket_remote_ip_(Socket& socket)
{
    ...
    if (ec)
    {
        LOG_ERROR("get socket remote address failed {}", ec.message());
        return "";
    }
    std::string ip = ed.address().to_string(ec);
    if (ec)
...
static uint16_t get_socket_remote_port_(Socket& socket)
{
    ...
    if (ec)
    {
        LOG_ERROR("get socket remote port failed {}", ec.message());
        return 0;
    }
    return ed.port();
}
...
boost::asio::ip::tcp::socket change_socket_io_context(boost::asio::ip::tcp::socket sock, boost::asio::io_context& io)
{
    ...
    if (ec)
    {
        LOG_ERROR("local {} remote {} change context failed {}", local, remote, ec.message());
        return tmp;
    }
