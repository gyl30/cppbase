...
{
   public:
    tcp_server(uint16_t port, std::string name, executors::executor &ex, executors &pool) : port_(port), name_(std::move(name)), acceptor_(ex), pool_(pool)
    {
        LOG_INFO("{} server :{} create", name_, port_);
    }

    ~tcp_server() { LOG_INFO("{} server :{} destroy", name_, port_); }
